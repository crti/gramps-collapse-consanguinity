# Consanguinity - Determine relationships between ancestors and spouses
#
...
#--------------------------------#
#                                #
# ConsanguinityGramplet class    #
#                                #
#--------------------------------#
class ConsanguinityGramplet(Gramplet):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.consanguinity_section = None
    ...
    def build_gui(self):
        ...
        self.consanguinity_section = Gtk.Label(label='')
        self.consanguinity_section.set_valign(Gtk.Align.START)
        self.consanguinity_section.set_halign(Gtk.Align.START)
        self.consanguinity_section.set_justify(Gtk.Justification.LEFT)
        self.consanguinity_section.connect('activate-link',
                                           self.on_activate_link)
    ...
    def refresh_content(self):
        ...
        parent = self.consanguinity_section.get_parent()
        parent.remove(self.consanguinity_section)
    ...
    def _make_content_widgets(self):
        ...
        if self.view_style == VIEW_STYLE_SINGLE:
            ...
            single_box.pack_start(self.consanguinity_section, False, False, 0)
        ...
        pedcoll_scrolled = Gtk.ScrolledWindow()
        pedcoll_scrolled.add(self.consanguinity_section)
        window.append_page(pedcoll_scrolled,
                           Gtk.Label(label=MSG_SPOUSAL_CONSANGUINITY))
    ...
    def on_copy_to_clipboard(self, button):
        ...
        if self.view_style == VIEW_STYLE_SINGLE:
            text += self.ped_collapse_section.get_text() + \
                   self.consanguinity_section.get_text()
        elif self.tab_displayed == 0:
            text += self.ped_collapse_section.get_text()
        else:
            text += self.consanguinity_section.get_text()
    ...
    def main(self): # return false finishes
        ...
        if cached_data:
            ...
            self.consanguinity_section.set_markup(cached_data[2])
        ...
        # Get spousal consanguinity section
        consanguinity_section = data_formatter.get_consanguinity()
        self.consanguinity_section.set_markup(consanguinity_section)

        # Save data in cache
        self.cache.add(active_handle,
                       (title, ped_coll_section, consanguinity_section,
                        self.pedigrees))
